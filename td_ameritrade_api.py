...
def place_order(account_num, access_token, symbol, asset_type, instruction, price, quantity):
    for a in range(0, COMMUNICATION_RETRY_TIMES+1):
        quote_data = get_quote(access_token=access_token, symbol=symbol)
        if instruction.upper() == 'BUY':
            if price > quote_data["{}".format(symbol)]["askPrice"] * 1.02:
                submit_price = quote_data["{}".format(symbol)]["askPrice"] * 1.02
            else:
                submit_price = price
        elif instruction.upper() == 'SELL':
            if price < quote_data["{}".format(symbol)]["bidPrice"] * 0.98:
                submit_price = quote_data["{}".format(symbol)]["bidPrice"] * 0.98
            else:
                submit_price = price
        else:
        ...
        if int(content.status_code) == 201:
            ...
            atstr = after_time.strftime("%Y-%m-%dT%H:%M:%S")
            a_time = datetime.strptime(atstr, "%Y-%m-%dT%H:%M:%S")
            ...
            if int(content.status_code) == 200:
                ...
                for order in data:
                    ...
                    if order['session'] == 'SEAMLESS' and order['orderType'] == 'LIMIT' and order['duration'] == 'DAY' and \
                            order['orderLegCollection'][0]['instrument']['assetType'] == asset_type and \
                            order['orderLegCollection'][0]['instrument']['symbol'] == symbol and "{:.2f}".format(
                        float(order['price'])) == "{:.2f}".format(submit_price) and order['orderLegCollection'][0][
                        'instruction'].upper() == instruction.upper() and \
                            int(order['orderLegCollection'][0]['quantity']) == int(
                        quantity) and entered_time >= b_time and entered_time <= a_time:
